...
export function Logo({ size = 36, withWordmark = true, tagline = false, stacked = false }: Props) {
  const { c, name } = useTheme();
  const dark = name === 'dark';
  const ink = dark ? '#F3F2F8' : '#0E122E';
  ...
  const wordmark = withWordmark ? (
    <View style={{ alignItems: stacked ? 'center' : 'flex-start' }}>
      <Text
        style={{
          fontWeight: '700',
          fontSize: stacked ? size * 0.42 : size * 0.6,
          letterSpacing: stacked ? -0.8 : -0.5,
          color: ink,
          textTransform: 'none',
        }}>
        Fringe
      ...
      {tagline ? (
        <Text
          style={{
            fontSize: stacked ? 10 : 9,
            letterSpacing: 1.6,
            color: c.accent,
            marginTop: stacked ? 6 : 4,
            fontWeight: '600',
          }}>
          TRACK · PLAN · SAVE
        </Text>
